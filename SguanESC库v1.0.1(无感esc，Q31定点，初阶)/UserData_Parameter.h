...
static inline void User_Parameter_Init(SguanESC_System_STRUCT *user){
    ...
    // +---------------------------------------------------------+
    // 1.低通滤波器LPF相关
    user->lpf.LPF_Sign.Wc = 10.0f;                  // 输入信号速率限幅

    // 2.无感速度解算相关
    user->encoder.LPF_Max = 1e10f;                  // LPF截止频率动态Wc变化上限限幅
    user->encoder.LPF_Min = 618.8f;                 // LPF截止频率动态Wc变化下限限幅

    user->encoder.PLL_K = 1.5f;                     // PLL动态截止频率倍率
    user->encoder.PLL_Max = 8.8f;                   // PLL截止频率Wc下限限幅
    user->encoder.PLL_Min = 1024.8f;                // PLL截止频率Wc下限限幅

    // 2.速度环PID参数
    user->pid.Wc = 100.0f;                          // 速度环speed的PID->微分滤波
    user->pid.Kp = 0.06f;                           // 速度环speed的PID->Kp
    user->pid.Ki = 0.4f;                            // 速度环speed的PID->Ki
    user->pid.Kd = 0.0f;                            // 速度环speed的PID->Kd
    user->pid.OutMax = 10.5f;                       // 速度环speed的PID->最大限幅
    user->pid.OutMin = -10.5f;                      // 速度环speed的PID->最小限幅
